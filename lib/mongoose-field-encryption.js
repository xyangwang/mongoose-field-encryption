...
const fieldEncryption = function (schema, options) {
  ...
  function encryptFields(obj, fields, secret) {
    ...
    for (const part of nestedSchemaes) {
  ...
  function updateHook(_next) {
    ...
    for (const part of nestedSchemaes) {
      const path = part.path;
      const schema = part.schema;
      const value = mpath.get(path, this._update);

      let result = null;

      if (value) {
        _.merge(value, mpath.get(path, this._update.$set));
      ...
      if (result) {
        mpath.set(path, result, updateObj);
      }
    }
  ...
  function loopNestedSchemaes(_schema, _path) {
    ...
    for (const _key of _keys) {
      ...
      loopNestedSchemaes(_value, _chain);
    }
  }
  ...
  function getSchema(_value) {
    ...
    if (_schema.obj && _schema.paths && _schema.tree) {
      return _schema;
    }
  ...
  function hasEncryptFunc(schema) {
    if (schema && schema.methods && schema.methods.encryptFieldsSync) {
      return typeof schema.methods.encryptFieldsSync === "function";
    }

    return false;
  }

  function hasDecryptFunc(schema) {
    if (schema && schema.methods && schema.methods.decryptFieldsSync) {
      return typeof schema.methods.decryptFieldsSync === "function";
    }
